...
export function execute(message: Message, args: string[]) {
    ...
    let subs = rcfg.subs.filter((sub: { name: string, channels: string[], hooks: string[] }) => {
        return sub.channels.includes(message.channel.id) || sub.hooks.includes(rcfg.webhooks[message.channel.id])
    });
